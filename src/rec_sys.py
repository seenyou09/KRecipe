...
class rec_sys:
    def __init__(self, N, n_ingred, category=None, df=None):
        self.numberOfRec = N
    ...
    def get_recommendations(self):
        ...
        recommendation = pd.DataFrame(columns=['EnglishName', 'KoreanName','Ingredients', 'score', 'url', 'category'])
        count = 0

        # Check if self.category is None or an empty list
        if self.category is None or not self.category:
            # If self.category is None or an empty list, consider all categories
            for i in top:
                recommendation.at[count, 'EnglishName'] = self.df_recipes['English Name'][i]
                recommendation.at[count, 'KoreanName'] = self.df_recipes['Korean Name'][i]
                ...
                category_value = str(self.df_recipes['Category'][i])[1:-1].replace("'", " ").replace(" ", " ")
                recommendation.at[count, 'category'] = category_value
                count += 1
        else:
            # If self.category is specified, filter recommendations based on categories
            for i in top:
